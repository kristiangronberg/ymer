...
for a client that has no skill to read. This skill is the grammar's home
in both registers — the prose below, and the description the fourth
statement writes — and this is the table's shape:
...
CREATE TABLE frictions (
  ...
  kind        TEXT    NOT NULL DEFAULT 'friction'
                      CHECK (kind IN ('friction','recurrence','vision','empty')),
  body        TEXT    NOT NULL,
...
INSERT OR REPLACE INTO _meta (target, description) VALUES ('table:frictions', 'The frictions store: one row per friction, a hindsight observation of process waste recorded as its root cause, never the symptom, blameless (name the artifact or system). Kaizen''s backlog, the store its skills moved onto on 2026-09-14; kaizen is today''s main producer (the kaizen:capture skill at Claude Code session tails), Cowork sessions insert by hand, and any client may insert. Minimal insert: source, context, body; captured_on (YYYY-MM-DD), kind and status default. source = the skill that captured it (brainstorm, implement, plan-review, ...), ''standalone'', or the surface (''cowork''). context = what the work belonged to: <repo>/<topic>, <skill>@<subject>, or a product name (ymer-node). kind: friction (default, one per capture) | recurrence = the same root cause biting again: set anchor_id to the row it recurs (an enforced foreign key; drained rows still count), or anchor_text when no single row is the anchor (a named-cause slug, or a pre-table key ''<date> · <source>@<context>''), body then carries at most one short where-it-bit clause | vision = material about where a product is heading rather than how the work went, product names it, several per session allowed | empty = a capture that found nothing, body ''∅ no friction'', never drained, excluded by kind. status: open | drained. Only /kaizen:summary drains: it sets status=''drained'', drained_to = the topic folder, task id or friction-batch that took the row, and drained_at. Rows are never deleted: pointers must keep resolving and drained rows stay as evidence. Frequency = rows sharing an anchor (COALESCE(anchor_id, anchor_text)); a recurrence after its anchor was drained is the sharpest signal kaizen produces. Grammar''s home: the kaizen:capture skill (ymer-marketplace/plugins/kaizen).');
```
...
worked around — no fallback file, no second store, no retry loop. A
notebook without the table gets it once, by you, from the definition
above — four `execute` calls, one statement each, since a call runs
only its first statement and drops the rest without a word; capture
never creates it.
...
A friction row is three values; the table fills in the rest — `id`,
...
6. **Record the row** — the capture's one friction, or the empty row —
   ...
   ```sql
   INSERT INTO frictions (source, context, body)
   VALUES ('<source>', '<context>', '<friction>')
   ```

   The other kinds name their extra columns:

   ```sql
   INSERT INTO frictions (source, context, kind, body)
   VALUES ('<source>', '<context>', 'empty', '∅ no friction')
   ```

   ```sql
   INSERT INTO frictions (source, context, kind, anchor_id, body)
   VALUES ('<source>', '<context>', 'recurrence', <anchor id>, '<where it bit>')
   ```

   ```sql
   INSERT INTO frictions (source, context, kind, product, body)
   VALUES ('<source>', '<context>', 'vision', '<product>', '<material>')
   ```
...
`<this skill's name>`. Battery, row grammar, and the write live in that
skill alone.
```
